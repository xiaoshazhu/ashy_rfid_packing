...
class T63RSdk:
    ...
    def set_text_font(self, lc_hdl: int, font_name: str = "宋体", font_size: float = 10.0, is_bold: bool = False):
        ...
        if hasattr(self._dll, "DSTP2x_LcDraw_SetTextFontName"):
            try:
                ...
                if fname.lower() in ("simsun", "songti"):
                    fname = "宋体"
    ...
    def draw_text(
        ...
        approx_w_per_pt = (cjk_count * 0.353 + ascii_count * 0.195)
        if approx_w_per_pt > 0 and w > 0:
            max_pt = w / approx_w_per_pt
            if font_size > max_pt:
                font_size = max(6.0, round(max_pt, 1))

        text_width_mm = approx_w_per_pt * font_size
        draw_x = x
        draw_w = w
        if align_right and w > text_width_mm:
            draw_x = round(x + (w - text_width_mm), 2)
            draw_w = round(text_width_mm, 2)
        ...
        res = self._dll.DSTP2x_Lbl_DrawText(
            c_uint64(lc_hdl), c_double(draw_x), c_double(y), c_double(draw_w), c_double(h), text_bytes
        )
        if res != 0:
    ...
    def draw_image(
        ...
        """在画布上绘制品牌 Logo 图片 (对应 C API: DSTP2x_Lbl_DrawImage)"""
    ...
    def set_img_dpi(self, dev_hdl: int, dpi_type: int = 1):
        """设置打印 DPI 模式 (1: 203 DPI, 2: 300 DPI)"""
    ...
    def set_lc_prn_rotate(self, lc_hdl: int, rotate: int = 0):
        """设置整张标签画布旋转角度 (0: 0度, 90: 旋转90度, 180: 180度, 270: 270度)"""
        rot_val = 0
        if rotate in (90, 1):
            rot_val = 90
    ...
    def rfid_locate_label(self, dev_hdl: int) -> int:
        """执行 RFID 标签天线自动定位校准 (DSTP2x_RFID_LocateLabel)"""
