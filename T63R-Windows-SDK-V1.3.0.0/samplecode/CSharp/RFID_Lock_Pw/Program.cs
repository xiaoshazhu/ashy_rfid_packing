...
namespace RFID_Lock_Pw
{
    class Program
    {
        ...
        [DllImport("../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_RFID_ChangeAccessPassword(DEV_HDL ullDevHdl, string szOldPw, string szNewPw);

        [DllImport("../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_RFID_SetDestructionPassword(DEV_HDL ullDevHdl, string szPw);

        [DllImport("../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_RFID_LockOperate(DEV_HDL ullDevHdl, Int32 nRFIDArea, string szPw);

        [DllImport("../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_RFID_SetPasswordWithWrite(DEV_HDL ullDevHdl, Int32 nRFIDArea, string szPw);
        ...
        static void Main(string[] args)
        {
            ...
            uRet = DSTP2x_RFID_ChangeAccessPassword(dev_prt, "00000000", "12345678"); 
            if (uRet != 0)
            {
                Console.WriteLine("Change RFID access password failed，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            // Maybe you can set the destruction password，please use with caution
            // uRet = DSTP2x_RFID_SetDestructionPassword(dev_prt, "87654321"); 
            // if (uRet != 0)
            // {
            //     Console.WriteLine("Set destruction password failed，error code:[" + uRet.ToString() + "]");
            //     goto SAMPLE_END;
            // }

            //7. we want to lock the EPC area. Whether to use or not
            uRet = DSTP2x_RFID_LockOperate(dev_prt, 1, "12345678");
            if (uRet != 0)
            {
                Console.WriteLine("Lock RFID with EPC failed，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            //8. set the password which will be write the EPC data with. Whether to use or not
            uRet = DSTP2x_RFID_SetPasswordWithWrite(dev_prt, 1, "12345678");
            if (uRet != 0)
            {
                Console.WriteLine("Set the password with write failed，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            //9. Set the rfid data.
            uRet = DSTP2x_LcRfid_SetData(lc_prt, 1, 1, writeEPCData, writeEPCData.Length); //Write EPC data with the type of ascii
            if(uRet != 0)
            {
            ...
            uRet = DSTP2x_LcRfid_SetData(lc_prt, 2, 1, writeUSERData, writeUSERData.Length); //Write USER data with the type of ascii
            if (uRet != 0)
            {
            ...
            //10.Print the label and read the data of EPC and USER and TID.
            uRet = DSTP2x_PrintLc(dev_prt, lc_prt, szOutFile, ref pOutFileSize, 7, szOutRFID, ref pOutRFIDSize);//Write and read the data of EPC and USER
            if (uRet != 0)
            {
