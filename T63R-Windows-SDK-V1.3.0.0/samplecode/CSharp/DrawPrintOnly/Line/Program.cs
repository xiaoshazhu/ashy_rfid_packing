...
namespace Line
{
    class Program
    {
        ...
        [DllImport("../../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_Lbl_DrawLine(LC_HDL ullLcHdl, Double dbStartX, Double dbStartY, Double dbEndX, Double dbEndY, Int32 nLineWidth, Int32 nLineType);
        ...
        static void Main(string[] args)
        {
            ...
            //4.Create the label.
            uRet = DSTP2x_CreateLabelContext(100, 120, ref lc_prt);
            if (uRet != 0)
            {
            ...
            //5.Draw Line
            uRet = DSTP2x_Lbl_DrawLine(lc_prt, 5, 5, 90, 15, 15, 0); //Solid line
            if (uRet != 0)
            {
                Console.WriteLine("Draw line failed，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            uRet = DSTP2x_Lbl_DrawLine(lc_prt, 5, 25, 90, 35, 15, 1); //Dashed line
            uRet = DSTP2x_Lbl_DrawLine(lc_prt, 5, 45, 90, 55, 15, 2); //Dot line
            uRet = DSTP2x_Lbl_DrawLine(lc_prt, 5, 65, 90, 75, 15, 3); //Dash dot line
            uRet = DSTP2x_Lbl_DrawLine(lc_prt, 5, 85, 90, 95, 15, 4); //Dash double dot line
