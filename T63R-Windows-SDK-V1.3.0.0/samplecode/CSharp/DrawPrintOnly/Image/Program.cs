...
namespace Image
{
    class Program
    {
        ...
        [DllImport("../../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_LcDraw_SetImageRotation(LC_HDL ullLcHdl, Int32 nTemporary, Int32 nAngle, Int32 nAnchorPoint);
        
        [DllImport("../../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_LcDraw_SetImageHalftoneAlgo(LC_HDL ullLcHdl, Int32 nTemporary, Int32 nAlgorithm, Int32 nThreshold);
        ...
        static void Main(string[] args)
        {
            ...
            //Non essential interface. Set the image algorithm.
            uRet = DSTP2x_LcDraw_SetImageHalftoneAlgo(lc_prt, 0, 1, 180);
            if (uRet != 0)
            {
                Console.WriteLine("Failed to set image algorithm，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            //Non essential interface. Set the image rotation.
            uRet = DSTP2x_LcDraw_SetImageRotation(lc_prt, 0, 0, 0);
            if (uRet != 0)
            {
                Console.WriteLine("Setting image rotation failed，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            //5.Draw image.
            String imgPath = "../../samplecode/CSharp/DrawPrintOnly/Image/image.jpg";
