...
namespace Barcode
{
    class Program
    {
        ...
        [DllImport("../../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_LcDraw_SetBarCodeEcLvl(LC_HDL ullLcHdl, Int32 nTemporary, Int32 nErrorCorrectLevel);
        
        [DllImport("../../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_LcDraw_SetBarCodeExpl(LC_HDL ullLcHdl, Int32 nTemporary, Int32 nExplanation);
        
        [DllImport("../../../../../../lib/Win32/libDSThermal.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        public static extern UInt32 DSTP2x_LcDraw_SetBarCodeRotation(LC_HDL ullLcHdl, Int32 nTemporary, Int32 nAngle, Int32 nAnchorPoint);        
        ...
        static void Main(string[] args)
        {
            ...
            uRet = DSTP2x_LcDraw_SetBarCodeExpl(lc_prt, 0, 1);
            if (uRet != 0)
            {
                Console.WriteLine("Setting whether to include text failed，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            //Non essential interface.
            uRet = DSTP2x_LcDraw_SetBarCodeRotation(lc_prt, 0, 0, 0);
            if (uRet != 0)
            {
                Console.WriteLine("Setting barcode rotation failed，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            //5.Draw bar code.
            uRet = DSTP2x_Lbl_DrawBarCode(lc_prt, 0, 10, 40, 20, 20, "123456"); //data must be utf-8
            if (uRet != 0)
            {
            ...
            //Non essential interface.
            uRet = DSTP2x_LcDraw_SetBarCodeEcLvl(lc_prt, 1, 2);
            if (uRet != 0)
            {
                Console.WriteLine("Failed to set barcode error correction level，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }

            //5.Draw QR code
            uRet = DSTP2x_Lbl_DrawBarCode(lc_prt, 0, 50, 40, 40, 58, "123456"); //data must be utf-8
            if (uRet != 0)
            {
                Console.WriteLine("Draw QR code failed，error code:[" + uRet.ToString() + "]");
                goto SAMPLE_END;
            }
