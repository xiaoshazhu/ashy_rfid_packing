...
int main(int argc, char *argv[])
{
	...
	uiRet = DSTP2x_RFID_ChangeAccessPassword(devHdl, "00000000", "12345678");
	if (uiRet)
	{
		printf("DSTP2x_RFID_ChangeAccessPassword error, error code:%d\n", uiRet);
		goto end;
	}

	// Maybe you can set the destruction password，please use with caution
	// uiRet = DSTP2x_RFID_SetDestructionPassword(devHdl, "87654321");
	// if (uiRet)
	// {
	// 	printf("DSTP2x_RFID_ChangeAccessPassword error, error code:%d\n", uiRet);
	// 	goto end;
	...
	uiRet = DSTP2x_RFID_LockOperate(devHdl, 1, "12345678");
	if (uiRet)
	{
		printf("DSTP2x_RFID_LockOperate error, error code:%d\n", uiRet);
		goto end;
	}

	//8. set the password which will be write the EPC data with. Whether to use or not
	uiRet = DSTP2x_RFID_SetPasswordWithWrite(devHdl, 1, "12345678");
	if (uiRet)
	{
		printf("DSTP2x_RFID_SetPasswordWithWrite error, error code:%d\n", uiRet);
		goto end;
	}
