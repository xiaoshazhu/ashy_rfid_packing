...
int main(int argc, char *argv[])
{
	...
	//5.
	uiRet = DSTP2x_LcDraw_SetEllipseRotation(labelHdl, 0, 30, 0);
	if (uiRet)
	{
		printf("DSTP2x_LcDraw_SetEllipseRotation error, error code:%d\n", uiRet);
		goto end;
	}

	//6.
	uiRet = DSTP2x_Lbl_DrawEllipse(labelHdl, 10, 10, 20, 20, 5, 0, 0);
	if (uiRet)
	{
		printf("DSTP2x_Lbl_DrawEllipse error, error code:%d\n", uiRet);
		goto end;
	}
